...
class GPT4Tokenizer(RegexTokenizer):
    ...
    def __init__(self):
        ...
        mergeable_ranks = enc._mergeable_ranks  # dict[bytes, int]  bytes -> token value
        ...
        self.byte_shuffle = {i: mergeable_ranks[bytes([i])] for i in range(256)}  # char byte -> token value
        self.inverse_byte_shuffle = {v: k for k, v in self.byte_shuffle.items()}  # token value -> char byte
    ...
    def save_vocab(self, vocab_file):
        ...
        # build vocab being mindful of the byte shuffle: sequence is saved as order of permuted token ids
...
def test():
    stuff = tiktoken.get_encoding("cl100k_base")._mergeable_ranks
    first = dict(list(stuff.items())[:300])
    print(first)
